...
INSERT INTO tbl_product_01 values ('AA02','에소프레소',3500);
...
insert into tbl_salelist_01 values('"+saleno+"', '"+pcode+"', to_date('"+saledate+"', 'yyyy-mm-dd'), '"+scode+"',"+ amount +")

String sql = "insert into tbl_salelist_01 values('saleno', 'pcode', to_date('saledate', 'yyyy-mm-dd'), 'scode', amount)";

select tsl.saleno,tp.pcode,tsl.saledate,ts.scode,tp.name,tsl.amount, TO_CHAR(tp.cost * tsl.amount,'999,999,999,999') cost from tbl_product_01 tp INNER JOIN tbl_salelist_01 tsl ON tp.pcode = tsl.pcode INNER JOIN tbl_shop_01 ts ON tsl.scode = ts.scode

select tp.pcode, tp.name, TO_CHAR(SUM(tp.cost * tsl.amount),'999,999,999,999') cost from tbl_product_01 tp INNER JOIN tbl_salelist_01 tsl ON tp.pcode = tsl.pcode INNER JOIN tbl_shop_01 ts ON ts.scode = tsl.scode GROUP BY tp.pcode,tp.name ORDER BY 1;

select tsl.scode, ts.sname, TO_CHAR(SUM(tp.cost * tsl.amount),'999,999,999,999') cost from tbl_salelist_01 tsl INNER JOIN tbl_product_01 tp ON tsl.pcode = tp.pcode INNER JOIN tbl_shop_01 ts ON tsl.scode = ts.scode GROUP BY tsl.scode,ts.sname ORDER BY 1;